...
# Minimal scripts - Using System Terminal Commands
...
## Questions
1. How can we leverage statistical and time-frequency features extracted from separated vocal and audio tracks to build effective language recognition models? Specifically, how can traditional machine learning methods — ranging from classical classifiers on simple statistical summaries to Gaussian Mixture Models on richer time-frequency features — be applied in this context? What are the key benefits and limitations of these approaches, and how can careful feature engineering, feature integration, and thorough model evaluation improve the accuracy and robustness of language recognition systems?

2. How can user interaction data combined with basic track metadata and simple audio features be used to build an effective recommendation system using collaborative filtering and traditional machine learning methods? Furthermore, how can advanced audio features, dimensionality reduction, and clustering techniques improve personalized recommendations by better capturing user preferences and track characteristics from both vocal and non-vocal components?


## Analysis plan

### Preamble: Easy and Medium Paths
The Easy Path serves as a minimal, foundational implementation aimed at quickly establishing a baseline for language recognition using straightforward statistical features extracted from separated vocal and audio tracks. It relies on classical machine learning models that are simple to train and interpret.

The Medium Path is designed as a more advanced approach to be adopted if the Easy Path does not achieve satisfactory performance or if the problem demands richer, more detailed feature representations. This path leverages time-frequency representations like spectrograms and deep learning models (CNNs) to capture complex audio patterns from both vocal and non-vocal tracks, potentially improving accuracy and robustness.
-   A plan for answering each of the questions including the variables involved, variables to be created (if any), external data to be merged in (if any).

## 1. Data Preparation & Feature Extraction  
- Load separated vocal and instrumental audio tracks for each sample.  
- **Easy Path:** Extract statistical features (mean, variance, skewness, kurtosis, RMS energy, zero crossing rate, tempo, loudness) from vocal and audio tracks separately.  
...
- Encode any categorical metadata if available (e.g., one-hot encoding for language labels or artist).
...
- **Medium Path:** Train classical models suited for time-frequency data distributions — Gaussian Mixture Models (GMMs), Hidden Markov Models (HMMs) (optional), or classical classifiers on aggregated time-frequency features.  
- Perform hyperparameter tuning via grid search or random search where applicable.
...
- Use metrics: Accuracy, Precision, Recall, F1-score to measure performance.  
- Generate confusion matrices to identify specific language confusions and error patterns.  
- Conduct ablation studies to compare models trained on vocal-only features, audio-only features, and combined vocal + audio features.

---

# Putative Machine Learning Techniques  
...
## 1. Data Preparation & Feature Extraction
- Load user interaction data with track metadata (artist, genre, audio features).  
...
- **Easy path:** Collaborative filtering with K-Nearest Neighbors or Logistic Regression using metadata and user preferences.  
- **Medium path:** Content-based filtering with clustering (K-Means, Hierarchical) and tree-based classifiers (Random Forest, Gradient Boosting).  
...
- Offline validation using held-out test sets.  
- Analyze recommendation relevance and diversity.  
- Ablation studies on vocal-only vs combined vocal+audio features.
...
- **Content-Based Filtering:** Clustering (K-Means, Hierarchical), Random Forest, Gradient Boosting Machines.  
